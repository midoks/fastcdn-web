...
export class FormApi {
  ...
  async removeSchemaByFields(fields) {
    ...
    const filterSchema = schema.filter(
      (item) => !fieldSet.has(item.fieldName)
    );
    this.setState({
      schema: filterSchema
    });
  ...
  setState(stateOrFn) {
    if (isFunction(stateOrFn)) {
      ...
      this.store.setState(
        (prev) => mergeWithArrayOverride(stateOrFn, prev)
      );
    }
  }
  /**
    ...
    fieldMappingTime.forEach(
      ([field, [startTimeKey, endTimeKey], format = "YYYY-MM-DD"]) => {
        ...
        if (format === null) {
          ...
          const [startTimeFormat, endTimeFormat] = Array.isArray(
            format
      ...
      if (Array.isArray(fieldConfig)) {
        ...
        if (Array.isArray(fields)) {
          ...
          if (value === "") {
            ...
            originValues[fields] = value.split(
